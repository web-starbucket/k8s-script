...
| `hosts` | 需要刷新时（单机） | 仅按 conf 写 `/etc/hosts`、可选改主机名 |
| `hosts-all` | 管理机 | 远程一键刷新全部 master/worker 的 hosts |
...
| `reset` | 本机 | `kubeadm reset` + 清 CNI/kubelet；加 `--vip` 顺带清 HAProxy/Keepalived |
| `reset-all` | 管理机 | 按 conf **一键清理全部** master/worker（默认含 VIP 组件） |
...
| `reset` | `--vip` / `--clean-vip` | 同时停掉并清理 HAProxy、Keepalived、VIP 相关配置 |
| | `--yes` / `-y` | 跳过 5 秒等待 |
| `reset-all` | `--yes` / `-y` | 跳过 10 秒危险确认（**建议显式加**） |
| | `--keep-vip` | 只清 Kubernetes，保留 HAProxy/Keepalived |
| | `--only=<IP>` | 只清理某一台 |
| | `--dry-run` | 只列出将清理的节点 |
...
| `CALICO_REGISTRY` | `docker.m.daocloud.io` | Calico 组件（国内可用；勿用空的阿里云公共路径） |
| `CALICO_IMAGE_PATH` | `calico` | 镜像路径 |
| `QUAY_MIRROR` | `quay.m.daocloud.io` | tigera-operator |
...
- **新增节点**：conf 加一行 → 管理机 `hosts-all` / 新机 `prepare`→ `join-masters` 或 `join-workers`
- **删除节点**：`kubectl drain/delete` → 从表去掉 → 管理机 `hosts-all` → 重跑 `vip-all`
...
# 管理机一键刷新全部 master/worker（推荐，需已能 SSH）
sudo bash install-k8s-1.33.sh hosts-all
# sudo bash install-k8s-1.33.sh hosts-all --only=172.16.10.117
# sudo bash install-k8s-1.33.sh hosts-all --dry-run
```
...
1. 经 `GH_PROXY` 下载 yaml  
2. 把 **tigera-operator** 镜像从 `quay.io` 换成 `quay.m.daocloud.io`  
3. 在 Installation 中设置 `registry: docker.m.daocloud.io`（Calico 组件走国内）

```bash
# 在 Master-1（默认已用国内镜像，无需额外参数）
sudo bash install-k8s-1.33.sh cni-calico
```

默认镜像（国内可用；阿里云公共仓无完整 Calico）：

| 用途 | 地址 |
|------|------|
| tigera-operator | `quay.m.daocloud.io/tigera/operator` |
| calico 组件 | `docker.m.daocloud.io/calico/...` |
| K8s 控制面（仍用阿里云） | `registry.aliyuncs.com/google_containers` |

```bash
sudo bash install-k8s-1.33.sh cni-calico
```

自有阿里云 ACR：先把 `tigera/operator`、`calico/node` 等同步进去，再：

```bash
export CALICO_REGISTRY=registry.cn-hangzhou.aliyuncs.com
export CALICO_IMAGE_PATH=你的命名空间
export QUAY_MIRROR=registry.cn-hangzhou.aliyuncs.com
sudo -E bash install-k8s-1.33.sh cni-calico
```
...
在管理机（任意能 SSH 各节点、且能读 `k8s-nodes.conf` 的机器）执行：

```bash
# 预览将清理哪些节点
sudo bash install-k8s-1.33.sh reset-all --dry-run

# 一键清理全部 master + worker（默认同时清 HAProxy/Keepalived）
sudo bash install-k8s-1.33.sh reset-all --yes

# 只清 Kubernetes，保留 VIP 组件
# sudo bash install-k8s-1.33.sh reset-all --yes --keep-vip

# 只清某一台
# sudo bash install-k8s-1.33.sh reset-all --yes --only=172.16.10.118
```

单机清理：

```bash
sudo bash install-k8s-1.33.sh reset --yes --vip
```

清理后重建仍按「推荐安装总览」：`prepare` → `ssh-keys` → `vip-all` → `init` → `cni-calico` → `join-all`。

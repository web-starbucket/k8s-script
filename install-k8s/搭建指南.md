...
# K8s 脚本化搭建指南（kubeadm + VIP）

Ubuntu 24 · `install-k8s.sh` · 支持 **v1.30–v1.33**（默认 1.33）· 对外 API：`https://<VIP>:8443`
...
```bash
# 每台物理机
chmod +x install-k8s.sh && sudo bash install-k8s.sh prepare

# 管理机（通常 m1）
sudo bash install-k8s.sh ssh-keys
sudo bash install-k8s.sh vip-all

# 仅 m1
sudo bash install-k8s.sh init \
  --apiserver-advertise-address=172.16.10.114 \
  --control-plane-endpoint=172.16.10.100:8443
sudo bash install-k8s.sh cni-calico
sudo bash install-k8s.sh join-all --prepare
sudo bash install-k8s.sh status
```

`bash install-k8s.sh --help`
...
脚本内写死支持范围 **v1.30～v1.33**，默认 **1.33**。conf 只需填要安装的版本：

| 变量 | 含义 |
| --- | --- |
| `K8S_MINOR` | 安装的次要版本（`1.30`～`1.33`）；不填则默认 `1.33` |
| `KUBE_VERSION` | 可选，钉死 apt 包版本（如 `1.33.1-1.1`） |

优先级：环境变量 > conf > 默认 1.33。超出范围脚本拒绝执行，需自行安装。改版本后须全员重新 `prepare`（或先 `reset`）。
...
bash install-k8s.sh nodes          # 核对版本 / VIP / Master，须与真实网卡 IP 一致
```
...
```bash
sudo bash install-k8s.sh prepare
sudo bash install-k8s.sh ssh-keys
sudo bash install-k8s.sh hosts-all   # 改 conf 后
```
...
```bash
sudo bash install-k8s.sh vip-all
# sudo bash install-k8s.sh vip-all --iface=ens18
```
...
```bash
sudo bash install-k8s.sh join-all --prepare
# sudo bash install-k8s.sh join-masters --prepare
# sudo bash install-k8s.sh join-workers --reset --only=172.16.10.117
sudo bash install-k8s.sh status
```
...
```bash
sudo bash install-k8s.sh reset-all --dry-run
sudo bash install-k8s.sh reset-all --yes
```
...
sudo -E bash install-k8s.sh cni-calico
```

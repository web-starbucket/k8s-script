...
banner "Envoy Gateway 安装" "Gateway API ${GATEWAY_API_VERSION} · EG ${EG_VERSION}"
...
if [[ -n "${IMAGE_REGISTRY}" ]]; then
  ...
  cp "${WORKDIR}/eg-proxy.yaml" "${TMPDIR}/eg-proxy-local.yaml"
  info "使用官方镜像（未设置 IMAGE_REGISTRY）"
fi

while IFS= read -r img; do
  dim "${img}"
done < <(grep -E '^\s*image:' "${INSTALL_YAML}" "${TMPDIR}/eg-proxy-local.yaml" | sort -u || true)
cp "${INSTALL_YAML}" "${WORKDIR}/install-local-applied.yaml"
cp "${TMPDIR}/eg-crds.yaml" "${WORKDIR}/eg-crds.yaml"
ok "清单已写入 install-local-applied.yaml"

step 5 6 "安装 EG CRD + 控制器（严格串行，无整包）"
split_and_apply "${TMPDIR}/eg-crds.yaml" "${TMPDIR}/eg-crds-split" "Envoy Gateway CRD"
split_and_apply "${INSTALL_YAML}" "${TMPDIR}/eg-install-split" "EG 控制器清单"
ok "控制器清单已全部串行应用"

step 6 6 "数据面 EnvoyProxy + GatewayClass（串行）"
split_and_apply "${TMPDIR}/eg-proxy-local.yaml" "${TMPDIR}/eg-proxy-split" "EnvoyProxy / GatewayClass"

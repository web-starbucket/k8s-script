#!/usr/bin/env bash
# 将 Envoy Gateway 相关镜像同步到私有仓库（可选）
# 用法:
...
if [[ -z "${PRIVATE_REGISTRY:-}" ]]; then
  echo "请设置 PRIVATE_REGISTRY，例如: export PRIVATE_REGISTRY=registry.example.com/myproj"
  exit 1
fi
PRIVATE_REGISTRY="${PRIVATE_REGISTRY%/}"
DAO_MIRROR="${DAO_MIRROR:-docker.m.daocloud.io}"
...
banner "同步镜像到私有仓库" "${PRIVATE_REGISTRY}/<name>:<tag>"

need docker
tip "请先 docker login 到你的私有仓库"
kv "镜像数量" "${#IMAGES[@]}"
kv "优先镜像源" "${DAO_MIRROR}"
echo
...
for name_tag in "${IMAGES[@]}"; do
  idx=$((idx + 1))
  dest="${PRIVATE_REGISTRY}/${name_tag}"
...
done_ "同步完成"
echo "${CG}${CB}✨ 私有仓镜像列表${C0}"
echo "${CD}────────────────────────────────────────────────────────${C0}"
for img in "${IMAGES[@]}"; do
  dim "${PRIVATE_REGISTRY}/${img}"
done
echo "${CD}────────────────────────────────────────────────────────${C0}"
info "CRD（非镜像）:"
